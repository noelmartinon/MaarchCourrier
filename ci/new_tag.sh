...
if [ -z $IS_TMA ]; then
    ...
    sed -i -e "s/$CI_COMMIT_TAG/$next_tag/g" apps/maarch_entreprise/xml/applicationVersion.xml

    git add -f apps/maarch_entreprise/xml/applicationVersion.xml

    sed -i -e "s/$CI_COMMIT_TAG/$next_tag/g" sql/data_fr.sql

    git add -f sql/data_fr.sql

    sed -i -e "s/$CI_COMMIT_TAG/$next_tag/g" migration/20.03/2003.sql

    git add -f migration/20.03/2003.sql

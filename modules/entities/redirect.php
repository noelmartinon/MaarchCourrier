...
function get_form_txt($values, $path_manage_action,  $id_action, $table, $module, $coll_id, $mode )
{
    ...
    if (!empty($_SESSION['user']['redirect_groupbasket'][$_SESSION['current_basket']['id']][$id_action]['entities'])) {
        ...
        $frm_str .='</select>';
        $frm_str .='<script>new Chosen($(\'department\'),{width: "80%", disable_search_threshold: 10, search_contains: true,allow_single_deselect: true});document.getElementById("department").onchange();Event.fire($(\'department\'), "chosen:updated");</script>';

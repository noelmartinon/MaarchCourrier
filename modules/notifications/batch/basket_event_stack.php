...
while ($state <> 'END') {
    ...
    switch ($state) {
        ...
        case 'ADD_EVENTS':
            ...
            $stmt = $db->query(
                "select * from baskets WHERE flag_notif = 'Y'"
            );
                    ...
                    while ($line3 = $stmt3->fetchObject()) {
                        ...
                        while ($line4 = $stmt4->fetchObject()) {
                            ...
                            if (!empty($real_user_id)) {
                                $stmt6 = $db->query("SELECT user_id FROM notif_event_stack WHERE record_id = '".$line4->res_id."' and  event_info like '%".$line->basket_id."%' and (user_id = '".$line3->user_id."' OR user_id = '".$user_id."')");
                            } else {

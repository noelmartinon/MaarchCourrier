...
function serverPath($forDocumentServer) {
    return $forDocumentServer && isset($exampeUrl) && $exampeUrl != "" 
        ? $exampeUrl : ('http://' . $_SERVER['HTTP_HOST'].'');
...
function  getStoragePath($fileName, $userAddress = NULL) {
    $storagePath = trim(str_replace(array('/','\\'), DIRECTORY_SEPARATOR, $getStorage), DIRECTORY_SEPARATOR);
    $directory =  $_SESSION['config']['tmppath'];
...
//. getCurUserHostAddress($userAddress)
    $directory = $directory  . DIRECTORY_SEPARATOR;
...
function getVirtualPath($forDocumentServer) {
    $storagePath = trim(str_replace(array ('/','\\'), '/', $getStorage), '/');
    $storagePath = $storagePath != "" ? $storagePath . '/' : "";
	$storePath = '/maarch_onlyoffice/apps/maarch_entreprise/tmp';
//. getCurUserHostAddress() 
    $virtPath = serverPath($forDocumentServer) . $storePath  . '/';

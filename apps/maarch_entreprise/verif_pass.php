...
if (!empty($_SESSION['error'])) {
    ...
    $db->query(
        "UPDATE ".$_SESSION['tablename']['users']." SET firstname = ?, lastname = ?, phone = ?, mail = ? , change_password = 'N' where user_id = ?",
        array($tmp_fn, $tmp_ln, $_SESSION['user']['Phone'], $_SESSION['user']['Mail'], $_SESSION['user']['UserId'])
    );

    \User\models\UserModel::updatePasswordByUserId(['userId' => $_SESSION['user']['UserId'], 'password' => $pass2]);
